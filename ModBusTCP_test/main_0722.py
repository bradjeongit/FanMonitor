...
class Go_Fan():
    ...
    for i in range(0, 16):
        ...
        replace_fan_ip[i] = '172.27.0.' + str(df_replace.iloc[0][i])
    ...
    for i in range(0,16):
        ...
        replace_sock_list[i].connect((replace_fan_ip[i], port))
    ...
    def read_normal(self):
        ...
        for i in range(0,16):
            for j in self.index_id_range:
                ...
                if fan_type == 'G': # Fan type G:Good, R:Replaced, B:Broken
...
#                    print(msg, "ref")
    ...
    def operate_fan(self):
        ...
        for i in range(0,16):
            for j in self.index_id_range:
                fan_type = self.df_status.iloc[j][i]
                fan_id = self.df_replace.iloc[j][i]
                if fan_type == 'R': # Fan type O:Normal, N:Neo, B:Broken

                    check_cntr = struct.pack('>HHHBBBHH',
                                             0, 0,  # TID/ PID
                                             6, fan_id,  # Length / Unit ID
                                             read_cntr, 0, 9)  # Function Code / Data (Reg addr, no of Reg)
                    print(check_cntr)

                    try:
                        replace_sock_list[i].send(check_cntr)
                        rsp_cntr = replace_sock_list[i].recv(60)

                        speed_normal = struct.unpack_from('>HHHBBBH', rsp_cntr, offset=0)
                        ...
                        if (status_cntr[6] == 0):
                            ...
                            print('Fan is off now [', replace_fan_ip[i], ' ID:', fan_id, ']')

                            try:
                                replace_sock_list[i].send(onoff_cntr)
                                rsp_cntr = replace_sock_list[i].recv(60)
